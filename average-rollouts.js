...
(async () => {
    ...
    for (let [id, buckets] of Object.entries(experimentRanges)) {
        ...
        for (let [bucket, ranges] of Object.entries(buckets)) {
            let start = Math.max(ranges.map(range => range.start));
            let end = Math.min(ranges.map(range => range.end));
    ...
    for (let [id, buckets] of Object.entries(averageRollouts)) {
        ...
        for (let [bucket, range] of Object.entries(buckets)) {
            data.rollout.populations[0].position = data.rollout.populations[0].position.filter(position => !position.position && position.bucket !== -1).concat(data.rollout.overrides_formatted[0]?.position);
